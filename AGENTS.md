...
Before acting, read the current GitHub Issue. It controls the task objective, acceptance criteria, risk, allowed and forbidden paths, fixed test profile, and next action. Issue #63 is the record for the governance consolidation that introduced this guide.
...
Use only the normal worker, fixed-test, bounded-review, Draft PR, and CI path. Do not weaken path enforcement, review bounds, the no-auto-merge rule, or consequential-action gates.
