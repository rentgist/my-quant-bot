...
| --- | --- | --- |
| CEO (`준희`) | Set goals and make consequential product or external decisions | Delegate an action implicitly where explicit approval is required |
...
A Codex self-review fallback is allowed only where the existing automation permits it. It must be labeled honestly and must never be represented as independent Claude review. Review evidence must be durable and GitHub-visible with an explicit top-level `PASS` or `CHANGES_REQUESTED` verdict.
