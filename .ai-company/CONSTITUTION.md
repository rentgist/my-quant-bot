...
This layer must not become a second production worker, queue, lifecycle database, merge path, scheduler, or competing source of truth.

## Roles

- `준희` is the CEO with authority over product direction and consequential decisions.
...
- When existing automation permits a Codex read-only self-review fallback, durable evidence must identify it as `Codex self-review`; it is not independent Claude review.
...
7. Implementation review evidence must be durable and GitHub-visible with an explicit top-level verdict of `PASS` or `CHANGES_REQUESTED`; a model route or local filename alone is not review evidence.

## Sources of truth

Operational truth remains in the current GitHub Issue, its lifecycle, the actual PR diff, fixed-test evidence, durable review evidence, CI, and Issue #35 worker health. Chat history and `.ai-company/` files are explanatory context, not substitutes for those records.
