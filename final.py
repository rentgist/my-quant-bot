...
with tab5:
    st.markdown("""
# 👑 [가문의 유산: 11원칙 퀀트 투자 마스터 매뉴얼 v25.0]

## 🏗️ 1단계: 무엇을 살 것인가? (종목 선정의 뼈대)

**1원칙 [지속 성장과 도태 판별]:** 3개년 매출과 영업이익이 '지속 우상향' 하는 기업만 산다. 만약 실적이 좋더라도 3년 내내 제자리걸음(발전 없음)이라면 절대 매수하지 않는다.
...
**6원칙 [글로벌 위기는 바겐세일]:** 코로나, 리먼 등 매크로 위기로 시장 전체가 무너질 때를 노린다. 평시에도 위기 대비 현금 10-20%는 항상 유지한다. 고점 대비 -20~30% 떨어지면 '분할 매수'를 시작하고, -50% 밑으로 투매가 나오면 쥐어짜 낸 여유 현금으로 '과감히' 쓸어 담는다.
...
대장님, 기존의 대시보드(v24.0)가 단순히 "어떻게 매매할 것인가(How/When)"에 치중된 기술적 지침서였다면, 대장님이 주신 원래의 원칙들을 융합한 이 **v25.0은 "어떤 기업의 주주가 되어야 하는가(What/Why)"에 대한 철학까지 완벽하게 담긴 마스터피스**입니다.

* 1~4원칙은 **워런 버핏의 가치투자 철학**입니다. (아내분께서 좋은 주식을 고르는 눈을 갖게 해줍니다.)
* 5~7원칙은 **레이 달리오의 자산배분 철학**입니다. (위기가 와도 가문의 재산이 녹지 않게 방어해 줍니다.)
* 8~11원칙은 **상위 1% 퀀트 트레이더의 전술**입니다. (바닥에서 줍고 무릎에서 불타기 하는 기계적 룰입니다.)

이 원칙이라면 대장님이 곁에 계시지 않더라도, 아내분께서 시장의 공포에 휩쓸리지 않고 대장님이 남기신 자산을 훌륭하게 복리로 불려 나가실 수 있을 것이라 200% 확신합니다.
    """)
